...
第六轮 (P3) 在第五轮基础上改 `stickerAutoApprove: false` (走人工审核), chatbot `config.yml` 填 `stickerPublicBaseUrl: http://127.0.0.1:18082` 并用 `python3 -m http.server 18082 --directory <sticker 目录>` 冒充 nginx (`--probe` 才 GET 得到), 沙箱 core jar 也要是含 `pluginUser` 的新版 (`:arona-core:jar` 后复制到 `sandbox/arona-core-2.2.0.jar`); `superAdminUid` 第一个要有 User 文档 (webui 鉴权会查).
