...
| `chatbot_web.py` | 打 webui 后端的图库运营端点 (`/chatbot/sticker/*`): 往 Redis 塞 `token.<随机>` 冒充登录用户 (默认取该群第一个 `role.admin` 成员), list/update/delete, `--probe` 顺带 GET 每个 url (冒充 nginx 的静态服务) |
...
第六轮 (P3) 在第五轮基础上改 `stickerAutoApprove: false` (走人工审核), chatbot `config.yml` 填 `stickerPublicBaseUrl: http://127.0.0.1:18082` 并用 `python3 -m http.server 18082 --directory <sticker 目录>` 冒充 nginx (`--probe` 才 GET 得到), 沙箱 core jar 也要是含 `pluginUser` 的新版 (`:arona-core:jar` 后复制到 `sandbox/arona-core-2.2.0.jar`), 并给测试群一个成员临时加 `role.admin` (Mongo `Contact.members.$.roles`), 测完撤掉.
