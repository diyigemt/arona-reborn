...
| `fake_llm.py` | 本机假 OpenAI `/chat/completions`, 由同目录 `llm_mode.txt` 切换 reply/empty/invalid/silent/error/slow/hang/block/noaudit/slowaudit |
| `chatbot_state.py` | 查看/直写测试群的 chatbot 配置, 列出 `chatContext` / `chatNoop` 与 Redis `chatbot.noop.<gid>` 计数 |
| `round2.sh` / `round3.sh` | 2026-08-23 验证用的场景脚本, 作范例 |
