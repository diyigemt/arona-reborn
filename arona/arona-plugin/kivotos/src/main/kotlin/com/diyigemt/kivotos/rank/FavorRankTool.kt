...
object FavorRankTool {
  private val RankResourcePath by lazy {
    Arona.dataFolder("gacha")
  }

  // 每个排名卡片的宽度
  private const val rankWidth = 964

  // 每个排名卡片的高度
  private const val rankHeight = 106

  //四周的margin
  private const val cardMargin = 50f

  // 左侧个人信息的起始坐标
  private const val selfStartX = 43f + cardMargin
  private const val selfStartY = 73f + cardMargin
  private const val selfWidth = 400f
  private const val selfHeight = 800f  //完全体可以上950

  // 第一个排名卡片的起始X坐标
  private const val rankStartX = 500f + cardMargin

  // 第一个排名卡片的起始Y坐标
  private const val rankStartY = 23f + 50f  // [卡片] 23f | 50f |
  private const val BgWidth = 1600
  private const val BgHeight = 1200
...
//        val favor = FavorLevelExcelTable.findLevel(current)
  ...
  private suspend fun drawFavCard(
    ...
    val pairSecond = "${pair.second}/${next}"
